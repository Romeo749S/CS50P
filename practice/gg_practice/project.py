...
def player_ends(current_card):
    ...
    if vote == "y":
        ...
        player.sum_cards = card_sum(player.cards)
...
def dealer_ends():
    while True:
        ...
        if (dealer.sum_cards) < 16:
            ...
            time.sleep(1)
            dealer.sum_cards = card_sum(dealer.cards)
        else:
            return dealer.sum_cards


def card_sum(list1):
    sum1 = sum([player._dict[n] for n in list1])
    while True:
